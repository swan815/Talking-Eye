...
while(true)
do
    if(file_exists('vis/vqa_request'))
    then 
        print('VQA request detected.')
        

        -- specify the image and the question.
        -- load the image
        local img_path = 'vis/demo_img1.jpg'
        local question_path = 'vis/question.txt'
        local question_file = io.open(question_path, "r")
        io.input(question_file)
        question = io.read()
        io.close(question_file)
